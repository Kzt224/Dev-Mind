...
const formatNearDeadlineBody = (taskName, days) => {
    if (days <= 0)
        return `🚨 Task "${taskName}" is past its deadline! Time to fix it now!`;
    if (days === 1)
        return `⚠️ Heads-up! Task "${taskName}" is due tomorrow — give it a final push!`;
...
export const handleUserQuery = async (userId, rawQuery) => {
    try {
        ...
        if (q.includes('how many days') || q.includes('days left') || q.includes('deadline')) {
            ...
            if (!foundTask)
                return "I couldn't find that task. Provide a task id or its exact name in quotes.";
            const dl = daysLeft(foundTask.endDate);
            if (dl === null)
                return `Task \"${foundTask.name}\" has no deadline set.`;
            if (dl < 0)
                return `Task \"${foundTask.name}\" was due ${Math.abs(dl)} days ago.`;
            if (dl === 0)
                return `Task \"${foundTask.name}\" is due today.`;
