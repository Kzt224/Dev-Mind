...
export class UserService {
    ...
    async updateUser(id, data, io) {
        ...
        const notification = UserNoti.default || UserNoti;
    ...
    async updatePassword(id, password, io) {
        if (!password)
            return { status: 400, json: { message: "Password required" } };
        if (password.length < 8)
            return { status: 400, json: { message: "Password must be at least 8 characters" } };
    ...
    async uploadProfile(imageUrl, userId) {
        try {
            if (!imageUrl) {
                return {
                    status: 400,
