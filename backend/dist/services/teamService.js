...
export class TeamService {
    ...
    async createGroup(userId, data) {
        try {
            const { name } = data;
            if (!name)
                return { status: 400, json: { message: "Group name is required" } };
    ...
    async getAllGroup(userId) {
        ...
        try {
            ...
            if (!groups || groups.length === 0)
                return { status: 404, json: { message: "No group found" } };
    ...
    async getGroupDetail(groupId, userId) {
        ...
        try {
            if (!groupId)
                return { status: 400, json: { message: "Group Id is required" } };
            const result = await this.prisma.groupMember.findMany({
                where: { groupId: Number(groupId) },
                include: { user: { omit: { password: true } } },
            });
            const isAdmin = result.some((r) => r.role === "ADMIN" && r.userId === Number(userId));
            return {
                status: 200,
                json: {
                    result,
    ...
    async generateInviteLink(groupId, userId) {
        ...
        try {
            if (!groupId)
                return { status: 400, json: { message: "groupId is required" } };
            const token = uuidv4();
    ...
    async checkInviteToken(token, userId) {
        try {
            const invite = await this.prisma.invitation.findUnique({ where: { token: token } });
            if (!invite)
                return { status: 404, json: { message: "Invalid invite link" } };
    ...
    async userConnectWithInviteLink(userId, data, io) {
        try {
            ...
            if (!invitation)
                return { status: 404, json: { message: "Invalid or expired invite link." } };
    ...
    async GroupLeftRequest(data, userId, io) {
        try {
    ...
    async adminGroupLeft(chUserId, gpId, requestLeftUserId, io) {
        try {
    ...
    async userGroupLeft(userId, groupId, reqUserName, io) {
        try {
    ...
    async handleAcceptLeftGroup(requestId, joinRequest, status, io) {
        try {
            if (status === "ACCEPTED") {
    ...
    async handleAcceptedJoinGroup(requestId, joinRequest, status, io) {
        try {
            if (status === "ACCEPTED") {
    ...
    async sentMemberToFeedBack(userId, requestId, status, io) {
        try {
            if (!requestId || !status)
            ...
            if (!joinRequest)
                return { status: 404, json: { message: "Join request not found" } };
    ...
    async emitNoti(payload, io) {
        try {
            ...
            const notification = payload.type === "JOIN"
    ...
    async searchMember(userId, query) {
        try {
