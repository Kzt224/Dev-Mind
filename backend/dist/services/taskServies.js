...
export class TaskService {
    ...
    async updateModeTracker(tx, authorId, updated) {
        try {
    ...
    async modifyTaskByOwner(data, task, authorId, id) {
        try {
            ...
            const durationDays = Math.ceil((end.getTime() - start?.getTime()) / (1000 * 60 * 60 * 24));
            let status = this.calculateProgress(progress);
    ...
    async modifyTaskByAssignee(progress, task, authorId, id) {
        try {
            if (progress !== undefined) {
                if (progress > 100) {
                    return {
    ...
    async modifyTask(id, data, authorId, io) {
        try {
            const { progress } = data;
            const task = await this.prisma.task.findUnique({
                where: { id: Number(id) },
    ...
    async handleTaskNotifications(io, task, updateTask, authorId) {
        try {
            if (!io)
                return;
    ...
    async emitNoti(io, service, recipientId, modifierId, task, projectId) {
        try {
